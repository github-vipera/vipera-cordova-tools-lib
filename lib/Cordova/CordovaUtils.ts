...
export class CordovaUtils {
  ...
  public parseDeviceList(stringValue:string){
      if(!stringValue){
        return [];
      }
      var parts=stringValue.split('\n');
      var deviceList=this.scanDeviceParts(parts);
      Logger.getInstance().debug(deviceList.toString());
      return deviceList;
    }

    parsePlatformList(stringValue:string){
      if(!stringValue){
        return [];
      }
      var parts=stringValue.split('\n');
    ...
    parsePluginList(stringValue:string){
      Logger.getInstance().debug("parsePluginList");
      var listParts=stringValue.split("\n");
      var result=new Array();
      _.forEach(listParts,(item:string)=>{
        result.push(this.parsePluginRecord(item));
      });
      Logger.getInstance().debug("parsePluginsList results:", result);
      return result;
    }

    parsePluginRecord(record:string){
    ...
    public getPlatformPath(platform:string,basePath:string):string|null {
      var result = basePath;
      if(platform === "android"){
    ...
    isCordovaProject(rootProjectPath:string):boolean {
      return false; //TODO!!
    }
